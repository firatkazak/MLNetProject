...
/*### SearchResultData Class Summary

`SearchResultData` sınıfı, arama sonuçlarının çeşitli özelliklerini temsil eden ve ML.NET tarafından kullanılmak üzere hazırlanmış çok sayıda özelliğe (property) sahiptir. Bu özellikler, arama sonuçlarının değerlendirilmesi ve sıralanması için kullanılır. Her bir özellik, belirli bir veri sütunuyla ilişkilendirilmiş ve bu sütunlardaki verilerin doğru şekilde yüklenebilmesi için `ColumnName` ve `LoadColumn` öznitelikleri kullanılarak tanımlanmıştır.

#### Özellikler

- **Label**: Modelin tahmin etmeye çalıştığı hedef etiket.
- **GroupId**: Belirli bir grup içinde yer alan arama sonuçlarının gruplandırılması.
- **CoveredQueryTermNumber**: Sorgudaki terimlerin belirli doküman bölümlerinde (Anchor, Title, URL, WholeDocument, Body) kapsama sayısı.
- **CoveredQueryTermRatio**: Sorgudaki terimlerin belirli doküman bölümlerinde (Anchor, Title, URL, WholeDocument, Body) kapsama oranı.
- **StreamLength**: Doküman bölümlerinin (Anchor, Title, URL, WholeDocument, Body) uzunluğu.
- **Idf**: Doküman bölümlerinin (Anchor, Title, URL, WholeDocument, Body) ters belge sıklığı (Inverse Document Frequency).
- **SumTf**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) terim sıklığı toplamı.
- **MinTf, MaxTf, MeanTf, VarianceTf**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) terim sıklığına dair minimum, maksimum, ortalama ve varyans değerleri.
- **SumStreamLengthNormalizedTf**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) normalize edilmiş terim sıklığı toplamı.
- **MinStreamLengthNormalizedTf, MaxStreamLengthNormalizedTf, MeanStreamLengthNormalizedTf, VarianceStreamLengthNormalizedTf**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) normalize edilmiş terim sıklığına dair minimum, maksimum, ortalama ve varyans değerleri.
- **SumTfidf, MinTfidf, MaxTfidf, MeanTfidf, VarianceTfidf**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) TF-IDF değerlerine dair toplam, minimum, maksimum, ortalama ve varyans değerleri.
- **BooleanModel, VectorSpaceModel, Bm25**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) çeşitli bilgi getirme modelleri (Boolean, Vektör Uzayı, BM25) için değerler.
- **LmirAbs, LmirDir, LmirJm**: Doküman bölümlerindeki (Anchor, Title, URL, WholeDocument, Body) Dil Modeli tabanlı bilgi getirme (LMIR) için değerler.
- **NumberSlashInUrl, LengthUrl**: URL içindeki eğik çizgi sayısı ve URL uzunluğu.
- **InlinkNumber, OutlinkNumber**: Sayfanın iç ve dış bağlantı sayıları.
- **PageRank, SiteRank**: Sayfanın PageRank ve SiteRank değerleri.
- **QualityScore, QualityScore2**: Sayfanın kalite puanları.
- **QueryUrlClickCount, UrlClickCount**: Sorgu-URL tıklama sayısı ve URL tıklama sayısı.
- **UrlDwellTime**: Kullanıcının URL'de geçirdiği süre.

Bu sınıf, makine öğrenimi modeli tarafından sıralama ve değerlendirme için kullanılan kapsamlı bir özellik seti sunar.*/