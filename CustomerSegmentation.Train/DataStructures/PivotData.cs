...
//PivotData sınıfı, verileri pivot formatında tutar;
public class PivotData
{
    public float C1 { get; set; }//C1 - C32: Her teklif için müşteri tarafından alınan teklif sayısı.
    ...
    public float C32 { get; set; }//C1 - C32: Her teklif için müşteri tarafından alınan teklif sayısı.
    public string LastName { get; set; }//Müşterinin soyadı.

    //Veriyi CSV formatında stringe çevirir;
    ...
    {
        string columns = "C1,C2,C3,C4,C5,C6,C7,C8,C9," +
                      "C10,C11,C12,C13,C14,C15,C16,C17,C18,C19," +
                      "C20,C21,C22,C23,C24,C25,C26,C27,C28,C29," +
                      $"C30,C31,C32,{nameof(LastName)}";

        File.WriteAllLines(file, salesData.Select(s => s.ToString()).Prepend(columns));
    }
}
