...
//DataHelpers sınıfı, verileri ön işleyip kaydetmek için yardımcı metotlar sağlar;
public class DataHelpers
{
    //PreProcessAndSave: offersDataLocation ve transactionsDataLocation konumlarından verileri okuyup işledikten sonra pivotDataLocation konumuna kaydeder.
    ...
    {
        IEnumerable<PivotData> preProcessData = PreProcess(offersDataLocation, transactionsDataLocation);
        PivotData.SaveToCsv(preProcessData, pivotDataLocation);
        return preProcessData;
    }

    //PreProcess: Verileri okuyup işlemi gerçekleştiren metot;
    public static IEnumerable<PivotData> PreProcess(string offersDataLocation, string transactionsDataLocation)
    {
        ...
        IEnumerable<Offer> offers = Offer.ReadFromCsv(offersDataLocation);
        IEnumerable<Transaction> transactions = Transaction.ReadFromCsv(transactionsDataLocation);

        var clusterData = (from of in offers
                           join tr in transactions on of.OfferId equals tr.OfferId
                           select new
                           { of.OfferId, of.Campaign, of.Discount, tr.LastName, of.LastPeak, of.Minimum, of.Origin, of.Varietal, Count = 1, }).ToArray();

        PivotData[] pivotDataArray = (from c in clusterData
