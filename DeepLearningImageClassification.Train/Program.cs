...
using Common;
using DeepLearningImageClassification.Shared;
using DeepLearningImageClassification.Shared.DataModels;
using DeepLearningImageClassification.Train.DataModels;
...
void TrySinglePrediction(string imagesFolderPathForPredictions, MLContext mlContext, ITransformer trainedModel)
{
...
string GetAbsolutePath(string relativePath) => FileUtils.GetAbsolutePath(typeof(Program).Assembly, relativePath);
...
void ConsoleWriteImagePrediction(string ImagePath, string Label, string PredictedLabel, float Probability)
{
